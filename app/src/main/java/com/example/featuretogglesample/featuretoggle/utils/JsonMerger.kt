...
import com.example.featuretogglesample.featuretoggle.utils.JsonMerger.ArrayMergeMode
import com.example.featuretogglesample.featuretoggle.utils.JsonMerger.ArrayMergeMode.MERGE_ARRAY
import com.example.featuretogglesample.featuretoggle.utils.JsonMerger.ArrayMergeMode.REPLACE_ARRAY
import com.example.featuretogglesample.featuretoggle.utils.JsonMerger.ObjectMergeMode
import com.example.featuretogglesample.featuretoggle.utils.JsonMerger.ObjectMergeMode.MERGE_OBJECT
import com.example.featuretogglesample.featuretoggle.utils.JsonMerger.ObjectMergeMode.REPLACE_OBJECT
 ...
 * details
 */
class JsonMerger @JvmOverloads constructor(
    private val arrayMergeMode: ArrayMergeMode = REPLACE_ARRAY,
    private val objectMergeMode: ObjectMergeMode = MERGE_OBJECT
) {
     ...
     */
    private fun merge(baseJson: JSONObject, overrideJson: JSONObject): JSONObject =
    ...
    private fun mergeElement(
        base: Any?,
        newValue: Any,
        objectMergeMode: ObjectMergeMode = this.objectMergeMode
    ) = when (base) {
    ...
    private fun maybeOverrideMergeMode(
        override: Any,
        objectMergeMode: ObjectMergeMode
    ): ObjectMergeMode {
    ...
    private fun mergeArray(base: JSONArray, override: Any): JSONArray {
        return if (override is JSONArray) {
            when (arrayMergeMode) {
    ...
    private fun throwUnsupportedElements(baseJson: String, overrideJson: String, cause: Exception) =
        IllegalArgumentException(
            "Can only merge json objects or arrays. Base json was '$baseJson', override was '$overrideJson'",
            cause
        )

    /**
     * @see com.example.featuretogglesample.featuretoggle.utils.JsonMerger.merge(org.json.JSONObject, org.json.JSONObject)
     */
    fun merge(vararg jsonObjects: JSONObject): JSONObject {
        if (jsonObjects.isEmpty()) return JSONObject()
        if (jsonObjects.size == 1) return jsonObjects.first()
        val it = jsonObjects.iterator()
        var baseJson = it.next()
        while (it.hasNext()) {
            val overrideJson = it.next()
            baseJson = merge(baseJson, overrideJson)
        }
        return baseJson
    }
     ...
     * @see com.example.featuretogglesample.featuretoggle.utils.JsonMerger.merge(org.json.JSONObject, org.json.JSONObject)
     */
    fun merge(jsonObjects: List<JSONObject>): JSONObject {
        return merge(*jsonObjects.toTypedArray())
    }
}
